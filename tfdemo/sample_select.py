            ...
            for key in near_neighbors.keys():
                if key != currentLabel:
                    ...
                    for k in range(len(idn)):
                        # implementing the triplets mining
                        # dap = mining_triplets_distance(currentImage, images[idp[j]])
                        # dan = mining_triplets_distance(currentImage, images[idn[k]])
                        # and (dap + config.MARGIN) > dan
                        # if dap < dan:
